module.exports = [
    {
        "title": '数据分析',
        "collapsable": true,
        "children": [
            ['dgiot/dba/device/', "查询单个设备时序数据"]
            ],
        "category": "工业数据云平台"
    },
    {
        "title": 'TDengine',
        "collapsable": true,
        "children": [
            ['tdengine/00.index/docs', 'TDengine文档'],
            ['tdengine/01.evaluation/docs', 'TDengine 介绍'],
            ['tdengine/02.getting-started/docs', '立即开始'],
            ['tdengine/03.architecture/docs', '数据模型和整体架构'],
            ['tdengine/04.model/docs', '数据建模'],
            ['tdengine/05.insert/docs', '高效写入数据'],
            ['tdengine/06.queries/docs', '高效查询数据'],
            ['tdengine/07.advanced-features/docs', '高级功能'],
            ['tdengine/08.connector/docs', '连接器简介'],
            ['tdengine/08.connector/01.java/docs', 'Java Connector'],
            ['tdengine/09.connections/docs', '与其他工具的连接'],
            ['tdengine/10.cluster/docs', 'TDengine 集群安装、管理'],
            ['tdengine/11.administrator/docs', 'TDengine的运营与维护'],
            ['tdengine/12.taos-sql/docs', 'Sql示例'],
            ['tdengine/12.taos-sql/01.error-code/docs', '错误码'],
            ['tdengine/13.faq/docs', '常见问题']
        ],
        "category": "工业数据云平台"
    }
]
