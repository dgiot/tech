...
本次DG-IoT海量用电信息采集与管理平台测试使用开源压力测试工具虚拟电表模拟用智能电表接收平台数据情况，通过MQTT通信完成采集数据回传，对被测系统进行负载测试。如果在测试WEB应用过程中发现某些页面单次访问加载时间很慢则采用ChromeNetwork工具进行排查。对于服务器端使用Grafana服务直接通过目标拉取数据，或者间接地通过中间网关拉取数据实现。
